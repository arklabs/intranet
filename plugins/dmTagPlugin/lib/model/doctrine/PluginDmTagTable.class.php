...
class PluginDmTagTable extends myDoctrineTable
{
  ...
  public function getPopularTagsQuery($relations = null, $limit = null, dmDoctrineQuery $q = null)
  {
    ...
    $counts = array();
    foreach ($this->getRelationHolder()->getAssociations() as $asociation){
      $relation = $asociation->getAlias();
      ...
      foreach (Doctrine::getTable($asociation->getClass())->getRelations() as $name=>$rel){
          if ($rel->getAlias() == 'DmUser'){
             if (dmContext::getInstance()->getUser()->isAuthenticated())
	         $q->where($relation.'.'.$rel->getLocal(). ' = '.dmContext::getInstance()->getUser()->getDmUser()->getId());
          }
      }
    }
