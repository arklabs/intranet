...
foreach ($affiliatePager as $affiliate)
{
  $table->body(
  sprintf('<a href="%s"  rel="tipsy" original-title="%s" class="color-box-trigger"> %s</a>',_link('app:admin/+/'.$sfModule.'/edit')->params(array('pk'=> $affiliate->getId(),'dm_embed'  => 1))->getHref(), $affiliate->getUsername(), $affiliate->getFirstName().' '.$affiliate->getLastName()),
  ($affiliate->getPhone())?sprintf('<a href="%s"  rel="tipsy" original-title="%s" class="color-box-trigger"> %s</a>',_link('app:admin/+/'.$sfModule.'/edit')->params(array('pk'=> $affiliate->getId(),'dm_embed'  => 1))->getHref(), 'Clic para modificar el telÃ©fono de este afiliado', $affiliate->getPhone()):sprintf('<a href="%s"  rel="tipsy" original-title="%s" class="color-box-trigger"> %s</a>',_link('app:admin/+/'.$sfModule.'/edit')->params(array('pk'=> $affiliate->getId(),'dm_embed'  => 1))->getHref(), 'Clic para agregar el telÃ©fono de este afiliado', "No asignado"),
  ($affiliate->getAffiliateCompany())?sprintf('<a href="%s"  rel="tipsy" original-title="%s" class="color-box-trigger"> %s</a>',_link('app:admin/+/'.$sfModule.'/edit')->params(array('pk'=> $affiliate->getId(),'dm_embed'  => 1))->getHref(), 'Clic para modificar cambiar la compañía de este afiliado', $affiliate->getPhone()):sprintf('<a href="%s"  rel="tipsy" original-title="%s" class="color-box-trigger"> %s</a>',_link('app:admin/+/'.$sfModule.'/edit')->params(array('pk'=> $affiliate->getId(),'dm_embed'  => 1))->getHref(), 'Clic para asignar la compañía de este afiliado', "No asignado"),
  ...
  _open('div.hidden').
  get_partial('affiliate/affiliateActionPanel', array('affiliate'=>$affiliate))._close('div')
  );
}
...
<script type="text/javascript">
   var prepend = '<?php include_partial('affiliate/listHeader', array( 'sfModule'=>'affiliate', 'statusControl'=>''));?>';
   var append = '';
   ...
   function reload(){
       $('#loader').show();
       $('.affiliate_list > .dm_widget_inner').load('/index.php/+/main/renderComponent?mod=affiliate&name=list',{},function(){
            //initDatatable();
	    initButtonMenus();
            $('#loader').hide();
            loadNotifications();
