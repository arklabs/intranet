...
class eventPublicActions extends myFrontModuleActions
{
  public function executeGetEventBasics(sfWebRequest $request){
       ...
       $event = Doctrine::getTable('EventPublic')->findById($eventId);
       if (count($event)){
        $event = $event[0];
       }
  ...
  public function executeGetMyEvents(sfWebRequest $request)
  {
    try{
            ...
            $q = null;
            //$eventList = Doctrine::getTable('EventPublic')->createQuery()->execute();
            ...
            foreach ($eventList as $event){
                ...
                $eventDescription = array(
                                        ...
                                        'editable'=>(($this->getUser()->hasPermission('editPublicCalendarEvents_front') || $this->getUser()->isSuperAdmin())?true:false),
                                        //'url'=>'/index.php/+/event/newEvent?id='.$event->getId(), facebox mode
                                        'url'=>'/admin.php/+/eventPublic/edit/pk/'.$event->getId().'/dm_embed/1',
