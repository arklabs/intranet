...
class telemarkerComponents extends myFrontModuleComponents
{

  public function executeDatesByTelemarker()
  {
    $this->showColumns = array(
          'Agent'=>array('label'=>'Telemarcador', 'href'=>'', 'extra_clases'=>'', 'is_relation'=>0, 'type'=>'string', 'link_to_object'=>false),
      ...
      $agents = Doctrine::getTable('Telemarker')->createQuery()->execute();
  ...
  public function executeDatesByTelemarkerChart()
  {
    ...
    $lienzoHeight = 275;
    $textAngle =25;
    $this->FileName = "/cache/datesByTelemarkerChart.png";
    ...
    $DataSet->AddPoint($SerieX, "NombreTelemarcadores");
    ...
    $DataSet->SetAbsciseLabelSerie("NombreTelemarcadores");

    // Cache definition
    $Cache = new arkPCache(sfConfig::get('sf_root_dir').'/web/cache/');
    if($Cache->IsInCache("datesByTelemarkerChart",$DataSet->GetData())){
            $this->FileName = "/cache/".$Cache->GetHash("datesByTelemarkerChart",$DataSet->GetData());
    }
    else{
    ...
    $Test->setFontProperties(dmOs::join(sfConfig::get('dm_core_dir'), 'lib/vendor/pChart/Fonts/tahoma.ttf'),10);
    $Test->drawTitle(60,22,"Citas Asignadas y Finalizadas por Telemarcador",255,255,255,585);
    ...
    //$Test->AddBorder(2);
    $Cache->WriteToCache("datesByTelemarkerChart",$DataSet->GetData(),$Test);
    $this->FileName = "/cache/".$Cache->GetHash("datesByTelemarkerChart",$DataSet->GetData());
    $Test->Render(sfConfig::get('sf_root_dir').'/web/cache/'.$Cache->GetHash("datesByTelemarkerChart",$DataSet->GetData()));
