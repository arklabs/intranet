    ...
    $(document).ready(function(){
        ...
        $('#calendar-container').DatePicker({
         ...
         $('#load-dates-trigger').click(function(){
             $('#loader').show();
             $.ajax({
                 	 url: '/index.php/+/main/renderComponent?mod=eventFeedBack&name=list&date_start='+getRangeDateStart()+'&date_end='+getRangeDateEnd(),
                 	 success: function(response){
            	     	$('.list-container').html(response);
            	     	$('#loader').hide();
                     },
                     error: function(){
                         alert('Ha ocurrido un error mientras se obten�a la lista.');
                     }
                 });
         });
